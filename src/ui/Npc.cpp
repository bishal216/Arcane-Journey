...
void NpcManager::addNpc(float x, float floorY, NpcType type,
...
void NpcManager::build(float worldH) {
    ...
    addNpc(120.f, floorY, NpcType::Shop,       "Merchant",    sf::Color(200, 160,  40), sf::Color(230, 190,  80));
    addNpc(280.f, floorY, NpcType::Cosmetics,  "Tailor",      sf::Color(160,  60, 200), sf::Color(200, 120, 240));
    addNpc(512.f, floorY, NpcType::TimeKeeper, "Chronicler",  sf::Color( 40, 160, 200), sf::Color( 80, 200, 240));
    addNpc(740.f, floorY, NpcType::Lore,       "Seer",        sf::Color(180,  60,  60), sf::Color(220, 100, 100));
    addNpc(900.f, floorY, NpcType::WiseMan,    "Sage",        sf::Color( 60, 160,  80), sf::Color(100, 210, 130));
}
...
int NpcManager::getNearbyNpc(sf::Vector2f playerPos) const {
    for (int i = 0; i < (int)m_npcs.size(); ++i) {
        ...
        if (std::sqrt(dx*dx + dy*dy) < m_npcs[i].interactRadius)
            return i;
    }
    return -1;
}
...
void NpcManager::draw(sf::RenderWindow& window, const sf::Font& font) const {
    ...
    for (const auto& npc : m_npcs) {
        ...
        nameText.setOrigin({nb.position.x + nb.size.x / 2.f,
                            nb.position.y + nb.size.y});
        nameText.setPosition({npc.pos.x,
                              npc.pos.y - BODY_H - HEAD_R * 2.f - 8.f + bob});
