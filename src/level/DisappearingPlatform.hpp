...
struct DisappearingPlatform {
    ...
    DisappearingPlatform(float x, float y, float w, float h,
                         sf::Color col = sf::Color(140, 80, 200), float visTime = DEFAULT_VISIBLE,
                         float hidTime = DEFAULT_HIDDEN, float fade = DEFAULT_FADE,
                         float offset = 0.f);
    ...
    bool isSolid() const {
        return state == DisappearState::Visible || state == DisappearState::FadingOut;
    }
...
class DisappearingPlatformManager {
   public:
    ...
    void add(float x, float y, float w, float h, sf::Color col = sf::Color(140, 80, 200),
             float visTime = DisappearingPlatform::DEFAULT_VISIBLE,
             float hidTime = DisappearingPlatform::DEFAULT_HIDDEN,
             float fade = DisappearingPlatform::DEFAULT_FADE, float offset = 0.f);

    void update(float dt);
    void draw(sf::RenderWindow& window) const;
    void reset();  // restore all platforms to initial visible state
