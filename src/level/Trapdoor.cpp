...
Trapdoor::Trapdoor(float x, float y, float w, float h,
...
bool Trapdoor::isSolidFromAbove() const {
    if (style == TrapdoorStyle::OneWay)
        return true; // always solid from above, never from below
...
void Trapdoor::update(float dt) {
    ...
    shape.setFillColor(sf::Color(
        (uint8_t)std::min(255.f, baseColor.r + glow),
...
bool TrapdoorManager::resolvePlayer(sf::Vector2f& playerPos,
                                     ...
                                     int&          jumpsLeft)
{
    ...
    for (auto& td : m_trapdoors) {
        ...
        td.onLand();
        g_discovery.discover(td.style == TrapdoorStyle::Hatch ? PlatType::Hatch : PlatType::OneWay);
