...
MovingPlatform::MovingPlatform(float x, float y, float w, float h, sf::Vector2f a, sf::Vector2f b,
...
void MovingPlatform::update(float dt) {
    ...
    if (dist < speed * g_artifacts.mods().platformSpeedMult * dt) {
        ...
        m_dir = toTarget / dist;
        shape.move(m_dir * speed * g_artifacts.mods().platformSpeedMult * dt);
    }

    velocity = m_dir * speed;
}

void MovingPlatform::draw(sf::RenderWindow& window) const { window.draw(shape); }

// ---------------------------------------------------------------------------
// MovingPlatformManager
// ---------------------------------------------------------------------------

void MovingPlatformManager::add(float x, float y, float w, float h, sf::Vector2f a, sf::Vector2f b,
...
bool MovingPlatformManager::resolvePlayer(sf::Vector2f& playerPos, sf::Vector2f& playerVel,
    ...
    for (auto& mp : m_platforms) {
        ...
        if (platMovedUp) effectiveOverlapTop -= std::abs(platDeltaY);  // shrink so top wins

        float minOverlap =
            std::min({effectiveOverlapTop, overlapBottom, overlapLeft, overlapRight});
        ...
        if (minOverlap == effectiveOverlapTop && playerVel.y >= -mp.speed - 10.f) {
            ...
            if (platDeltaY > 0.f) playerPos.y += platDeltaY;
